...
class PathfinderSampleEvent(EyeTrackerEvent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #: Dial value from Pathfinder biometrics kit, 0-1
        self.dial = None
        self.dialv = None
        #: GSR resistance value from Pathfinder biometrics kit, ohms
        self.gsr = None
        self.gsrv = None
        #: Heart rate value from Pathfinder biometrics kit, bpm
        self.hr = None
        self.hrv = None
        self.hrp = None
...
class EyeTracker(EyeTrackerDevice):
    """
    To start iohub with a Pathfinder Pathfinder eye tracker device, add a Pathfinder
    ...
    Examples:
        A. Start ioHub with Pathfinder Pathfinder device and run tracker calibration::
    ...
    EVENT_CLASS_NAMES = [
        ...
        'BlinkEndEvent']
    _recording = False
    __slots__ = ['_pathfinder', '_rx_buffer', '_ttfreq', '_last_fix_evt', '_serverIsLocalhost']
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.isConnected():
            try:
                self._pathfinderget("TIME_TICK_FREQUENCY")
                ack = self._waitForAck('TIME_TICK_FREQUENCY')
            except Exception:
                ack = None
            if ack and ack.get("FREQ") not in (None, ET_UNDEFINED):
                self._ttfreq = ack.get("FREQ")
    ...
    def setConnectionState(self, enable):
        ...
        if enable is True and self._pathfinder is None:
            try:
                ...
                init_connection_str += '<SET ID="ENABLE_SEND_OPENMM" STATE="1" />\r\n'
    ...
    def sendMessage(self, message_contents, time_offset=None):
        ...
        try:
            ...
            if self._pathfinder and self.isRecordingEnabled() is True:
                ...
                self._pathfinder.sendall(strMessage)
        except Exception:
    ...
    def runSetupProcedure(self, calibration_args={}):
        ...
        self._pathfinderset('CALIBRATE_TIMEOUT', VALUE=targ_timeout)
        self._pathfinderset('CALIBRATE_DELAY', VALUE=targ_delay)
        self._waitForAck('CALIBRATE_DELAY', timeout=2.0)

        if use_builtin is True:
            self._pathfinderset('CALIBRATE_RESET')
    ...
    def _parseFixationFromMsg(self, m, logged_time, tracker_time):
        ...
        if fix_valid == 1:
            ...
            time_val = m.get('TIME', ET_UNDEFINED)
            if time_val not in (None, ET_UNDEFINED):
                try:
                    time_val = int(time_val)
                except Exception:
                    time_val = ET_UNDEFINED
            time_tick = m.get('TIME_TICK', ET_UNDEFINED)
            if time_tick not in (None, ET_UNDEFINED):
                try:
                    time_tick = int(time_tick)
                except Exception:
                    time_tick = ET_UNDEFINED
            m = dict(FPOGID=fix_id, FPOGV=fix_valid, FPOGX=fix_x, FPOGY=fix_y, FPOGS=fix_stime, FPOGD=fix_duration,
                     TIME=time_val, TIME_TICK=time_tick)
    ...
    def _createStartFixEvt(self, m, logged_time, tracker_time):
        # Create start fixation evt based on m
        # Pathfinder does not create separate left and right eye fix evts, so we
        ...
        time_tick = m.get('TIME_TICK', ET_UNDEFINED)
        sample_delay = 0
        if self._serverIsLocalhost and self._ttfreq and time_tick not in (None, ET_UNDEFINED):
            try:
                evt_tick_time = int(time_tick)
