...
class PathfinderCalibrationProcedure(BaseCalibrationProcedure):
    ...
    def startCalibrationHook(self):
        ...
        self._eyetracker._pathfinderset('CALIBRATE_CLEAR')

        self._eyetracker._pathfinderset('SCREEN_SIZE', X=0, Y=0, WIDTH=self.width, HEIGHT=self.height)
        #print2err("Set Pathfinder SCREEN_SIZE: ", self._eyetracker._waitForAck('SCREEN_SIZE'))

        # Inform Pathfinder of target list to be used
        for p in self.cal_target_list:
            x, y = p
            self._eyetracker._pathfinderset('CALIBRATE_ADDPOINT', X=x, Y=y)
